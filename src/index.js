"use strict";
//1 task
function getTime(time, timeLate) {
    if (time + timeLate === 24) {
        return 0;
    }
...
//3 task
function getLastLengthWord(line) {
...
function getContrast(bright1, bright2) {
    return ((bright1 - bright2) / bright1) * bright1 > bright2;
}
//5 task
function validateBrackets(bracketsStroka) {
    const stack = [];
    const bracketMap = {
        ")": "(",
        "}": "{",
